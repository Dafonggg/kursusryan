...
@section('title', 'Tambah Materi - Instructor')
...
<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
	<div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
		...
		<div class="d-flex align-items-center gap-2 gap-lg-3">
			<a href="{{ route('instructor.materials', $course->slug) }}" class="btn btn-sm fw-bold btn-secondary">Kembali</a>
		</div>
	</div>
</div>
@endsection
...
<div class="card card-flush">
	...
	<div class="card-body pt-0">
		<form action="{{ route('instructor.materials.store', $course->slug) }}" method="POST" enctype="multipart/form-data">
			@csrf
			...
			<div class="row mb-10">
				...
				<div class="col-md-6">
					...
					<select name="scope" class="form-select @error('scope') is-invalid @enderror" required>
						...
						<option value="online" {{ old('scope') == 'online' ? 'selected' : '' }}>Online</option>
						<option value="offline" {{ old('scope') == 'offline' ? 'selected' : '' }}>Offline</option>
						<option value="both" {{ old('scope', 'both') == 'both' ? 'selected' : '' }}>Both</option>
					</select>
					@error('scope')
						<div class="invalid-feedback">{{ $message }}</div>
					@enderror
			...
			<div class="mb-10" id="file_upload_section">
				...
				<div class="form-text">Format: PDF, DOC, DOCX, PPT, PPTX, ZIP, RAR (Maks: 10MB)</div>
				@error('path')
					<div class="invalid-feedback">{{ $message }}</div>
				@enderror
			</div>
			...
			<div class="d-flex justify-content-end gap-2">
				<a href="{{ route('instructor.materials', $course->slug) }}" class="btn btn-light">Batal</a>
